...
export const metadata: Metadata = {
  ...
  description: `Intégrez un réseau sélectif de ${siteConfig.specialtyPlural} à ${siteConfig.cityLabel}. 1 praticien par zone × spécialité. Candidature examinée sous 48h.`,
}

const steps = [
  { n: '1', title: 'Envoyez votre candidature', desc: 'Remplissez le formulaire en 2 minutes — nom, spécialité, zone, email, téléphone.' },
    ...
    <>
      ...
      <div className="bg-green-dark text-center px-4 md:px-10 py-[52px]">
        <span className="inline-block bg-surface text-green-dark text-[10px] font-bold px-3.5 py-[5px] rounded-xl uppercase tracking-[1px] mb-4">
          Réseau sélectif · 1 praticien par zone
        </span>
        ...
        <p className="text-sm text-white/70 max-w-[560px] mx-auto leading-[1.65]">
          Un réseau fermé, sélectif et exclusif. Chaque praticien est le seul référencé
          dans son quartier pour sa spécialité — zéro concurrence directe garantie.
        </p>
      </div>
      ...
      <div className="bg-green-deep border-b border-white/10 py-5 px-6 md:px-10">
        <div className="max-w-[1060px] mx-auto flex flex-wrap justify-center gap-x-8 gap-y-3 md:gap-[60px]">
          {[
            ['✓ Vérification sous 48h', 'Certification & activité vérifiées'],
            ['✓ Profil indexé sur Google', 'Visible immédiatement sur la recherche locale'],
            ['✓ Réseau local exclusif', '1 seul praticien par zone × spécialité'],
          ].map(([n, l]) => (
            <div key={String(l)} className="text-center">
              <span className="block text-[15px] md:text-[17px] font-extrabold text-green-light">{n}</span>
      ...
      <section className="py-14 px-4 md:px-10">
        <div className="max-w-[560px] mx-auto">
          ...
          <div className="bg-white rounded-2xl p-7 border-2 border-green shadow-lg">
            ...
            {/* Exclusivity badge */}
            <div className="rounded-xl px-4 py-3 mb-6 bg-green/8 border border-green/20">
              <div className="flex items-center gap-2 mb-1">
                <span className="w-2 h-2 rounded-full bg-green shrink-0"></span>
                <p className="text-[12px] font-bold text-green-dark">Exclusivité zone × spécialité garantie</p>
              </div>
              <p className="text-[11px] text-muted leading-[1.6]">
                1 seul praticien accepté par quartier pour votre spécialité. Vérifiez la disponibilité dans le formulaire.
              </p>
            </div>
      ...
      <section className="py-[52px] px-4 md:px-10">
        <div className="max-w-[680px] mx-auto">
          ...
          {[
            ['Comment fonctionne l\'exclusivité de zone ?', `Chaque praticien est le seul référencé dans son quartier pour sa spécialité et sa niche. Lors de la candidature, vous choisissez votre zone et précisez votre approche — si la combinaison est libre, elle vous est réservée.`],
            ['Que se passe-t-il si ma zone est déjà prise ?', `Le formulaire vous l'indique en temps réel. Vous pouvez alors choisir une autre zone disponible, ou affiner votre niche pour vous différencier (ex. "Stress & burn-out" vs "Périnatal & maternité").`],
      ...
      <section className="bg-green-dark text-center py-[52px] px-4 md:px-10">
        ...
        <p className="text-[13px] text-white/70 mb-6">
          Vérifiez la disponibilité de votre zone et soumettez votre candidature — examinée sous 48h.
        </p>
        <a
          href="/inscription/formulaire"
